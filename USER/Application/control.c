...
/*****************************************************************************************************/
pid_t*(pPidObject[]) = {
    &pidRateX, &pidRateY, &pidRateZ, &pidRoll, &pidPitch, &pidYaw  //结构体数组，将每一个数组放一个pid结构体，这样就可以批量操作各个PID的数据了  比如解锁时批量复位pid控制数据，新手明白这句话的作用就可以了
};

/**
 * @brief 飞行器PID控制主函数，根据当前状态执行不同的控制逻辑。
 * 
 * 该函数通过状态机的方式管理飞行器的控制流程，包括等待解锁、准备控制、
 * 正式控制和退出控制等阶段。在控制过程中会进行角度检测、PID计算以及串级控制。
 *
 * @param dt 控制周期（单位：秒），用于PID积分和微分项的计算
 */
void FlightPidControl(float dt) {
    ...
    case PROCESS_31:                                                                     //正式进入控制
      if (Angle.pitch < -50 || Angle.pitch > 50 || Angle.roll < -50 || Angle.roll > 50)  //倾斜检测，大角度判定为意外情况，则紧急上锁
        if (Remote.thr > 1200)                                                           //当油门的很低时不做倾斜检测
          flagState.unlock = EMERGENT;                                                   //打入紧急情况
  ...
  if (flagState.unlock == EMERGENT)  //意外情况，请使用遥控紧急上锁，飞控就可以在任何情况下紧急中止飞行，锁定飞行器，退出PID控制
    status = EXIT_255;
}
...
void MotorControl(void) {
  ...
  switch (status) {
    ...
    case PROCESS_31: {
      int16_t temp, thr;
      temp = Remote.thr - 1000;  //油门+定高输出值，油门值（1000-2000）
      //油门比例规划
      thr = 100 + 0.55f * temp;
  ...
  Motor_SetPWM(MOTOR1, 1);  //更新PWM
  Motor_SetPWM(MOTOR2, 2);
  Motor_SetPWM(MOTOR3, 3);
  Motor_SetPWM(MOTOR4, 4);
}
